...
| **里程碑 2** | 📋 范围已定（记忆+信仰+人族+遗迹梦境），**实施计划未写，待做** |
...
**MVP-2 最小可玩增量**：
- 记忆（梦珀）资源 + 遗迹系统（数据驱动：遗迹 id/名称/梦境碎片文本/奖励）
- 根须探索（深度解锁遗迹，消耗树液）
- 信仰资源（人族献梦产生）
- 人族（说书人）唤醒事件（记忆≥2）
- 梦境碎片文本 3-4 个（诗歌化文风）
- UI：记忆/信仰显示、根须探索、梦境弹层

**暂不做（里程碑 3+）**：林地民/石裔/野民、人口 S 曲线、记忆图书馆、明选、奇迹。
**数值**：沿用 spec §14 骨架（记忆≥2 唤醒人族；遗迹一次性记忆；信仰=献梦产出）。

**实施流程**（按 DSH 编程模式）：writing-plans 写里程碑 2 实施计划 → 主人确认 → SDD 执行（subagent 实施 + 审查循环）。
