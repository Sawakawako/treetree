...
| **里程碑 8（M8）** | ✅ 完成（统一主玩法主壳 + 资源栏 + 四页渐进式导航 + 事件队列/阻塞层 + 固定头尾；55 套件 / 596 测试全绿，13 张 420×640 状态截图与 Agent Vision 复查通过，2026-09-08 封板） |
...
tests/unit/                 # GdUnit4 当前实测：596 测试，55 套件（M8 封板；M7 基线 513/46）
```
...
**下一步**：ROADMAP 1—10 已全部完成。继续开发前先由主人冻结新的产品方向；可选候选（仅供讨论，不视为已批准范围）包括真人平衡试玩、发布/导出准备、无障碍与多语言扩展。

## 七·七、里程碑 8 已封板（2026-09-08）

**主玩法界面视觉与信息架构重构完成**（按 `docs/superpowers/plans/2026-09-07-m8-main-ui-redesign.md`）：
- **统一主壳**：`features/ui/main.gd` / `main.tscn` 组装资源栏、返回标题、`PageStack`、底部导航与 `EventLayer`；全局 `GameManager` 信号集中在主壳，页面只通过 `UiPage` 请求刷新和反馈。
- **渐进式页签**：树心始终可见；众生、树语、九界分别由唤醒、树语进度、九界遗迹门槛解锁；页签切换保留焦点并使用短淡入，固定头尾不参与页面滚动。
- **事件与终局**：`UiEventQueue` 按终局/归还、明选、叙事、提示分层；`EventLayer` 负责阻塞层、焦点恢复、选择/归还/终局动作，原有 M6/M7 存档与终局语义未改动。
- **视觉验收**：Godot Vulkan 图形窗口按 420×640 实际运行，完成“标题→新的一轮→主玩法”真实入口链路；标题与主壳截图保存在忽略目录 `.gdskills/vision/`，并按 Taste Receptor Atlas 检查层级、对比度、字体、触控尺寸、暖纸色彩和安全区。首屏适用项无阻断级 0 分；主壳底部导航在 `y=568..616` 安全区内可见。

**M8 封板验证**：
- 静态检查：`git diff --check` 通过；`GameManager.*.connect` 仅在 `features/ui/main.gd`；Godot `--import --quit` 退出码 0。
- 全量 GdUnit4（无 `--add`，授权隔离 `user://`）：**55 套件 / 596 测试 / 0 error / 0 failure / 0 skipped / 0 orphan**。
- 主场景冒烟：Godot `--headless --path . --quit-after 5` 退出码 0，无 `SCRIPT ERROR`。
- 视觉证据：`20260908-022919-window-task9-title-foreground.webp`、`20260908-023006-window-task9-main-shell.webp`、`20260908-023240-window-task9-main-tail.webp`；临时布局探针已删除，截图与运行时目录均未入库。

**下一步**：ROADMAP 1—10 已全部交付。继续开发前先由主人冻结新的产品方向并完成设计；候选方向仅供讨论，不视为已批准范围：真人平衡试玩、发布/导出准备、无障碍与多语言扩展。
