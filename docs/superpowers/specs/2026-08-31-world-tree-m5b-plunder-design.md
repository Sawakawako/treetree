...
**揭示应用代价**（达 3/6/9 时）：
- 关系 -1（`RelationActions.apply_change(state, race_id, -1)`，每级累计 -3）
...
|---|---|---|---|---|
| 人族 | +1.0 记忆 | 3/6/9 | -1（累计 -3） | 人口冻结 |
| 林地民 | +1.2 记忆 | 3/6/9 | -1（累计 -3） | 人口冻结 |
| 野民 | +2.0 记忆 | 3/6/9 | -1（累计 -3） | 1 级：人口 -20%（惊扰攻击） |
| 石裔 | 0 | — | 无 | 无（夺无可夺） |

- 节奏对照：人族梦产 0.05/tick（50 人口）≈ 20 秒 +1 记忆；夺梦一次 +1（人族）瞬间获得——「快而伤」的诱惑成立
- 9 次日常夺梦 ≈ 菟丝子明选重夺（-3）的等价后果——日常的涓滴之恶积累成明选级之伤
...
| 套件 | 覆盖 |
|---|---|
| `test_plunder_actions.gd` | 产出各族不同（1/1.2/2/0）；石裔不涨 counter 且产出 0；counter 累积；阈值 3/6/9 揭示（reveal_stage 0→1→2→3）；揭示幂等（已揭示不重复应用）；关系扣减（每级 -1 累计 -3）；is_frozen（揭示后 true）；野民 1 级人口 -20% 一次 |
...
5. **代价以关系+冻结表达**：不侵入产出公式（M5b 简化）；9 次日常夺梦 ≈ 菟丝子 -3 等价。
...
- 菟丝子明选（重夺 -3 事件版）→ M5e 明选引擎，复用 PlunderActions 的代价机制
