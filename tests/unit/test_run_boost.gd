...
# —— 补充断言（brief 之外自审加强）：等级与树语节点赠予锁定 ——

func test_boost_run3_raises_levels_and_nodes() -> void:
	var s := GameState.new()
	s.run_number = 3
	RunBoost.apply_boost(s)
	...
	for n: StringName in RunBoost.BOOST_NODES:
		assert_that(s.lingua_nodes).contains(n)
	# 经济门控读 lingua_nodes：兑换/引擎在 run3 开局即解锁（lingua_nodes 非 choice_flags）
	assert_that(LinguaActions.has_node(s, &"tree_canopy")).is_true()
	assert_that(LinguaActions.has_node(s, &"grace")).is_true()

func test_boost_run3_keeps_higher_existing_levels_and_dedupes_nodes() -> void:
	# maxi 语义：已有更高等级不降；节点已存在（周目保留）不重复 append
	var s := GameState.new()
	s.run_number = 3
	s.leaf_level = 9
	s.root_depth = 9
	s.lingua_life_level = 9
	s.lingua_nodes.assign(RunBoost.BOOST_NODES)
	RunBoost.apply_boost(s)
	assert_that(s.leaf_level).is_equal(9)
	assert_that(s.root_depth).is_equal(9)
	assert_that(s.lingua_life_level).is_equal(9)
	assert_that(s.lingua_nodes.size()).is_equal(RunBoost.BOOST_NODES.size())
